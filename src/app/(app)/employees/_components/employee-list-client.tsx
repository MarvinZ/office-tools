  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {filtered.length !== employees.length && (
        <p className="text-sm text-zinc-400">{filtered.length} of {employees.length} employees</p>
      )}
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-sm text-zinc-400">No employees found.</td>
              </tr>
            ) : filtered.map((e) => (
              <tr key={e.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900/50">
                ...
                <td className="px-4 py-3 text-zinc-500">{e.departmentName ?? "—"}</td>
                <td className="px-4 py-3 text-zinc-500">{e.phone ?? "—"}</td>
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_CONFIG[e.status].color}`}>
