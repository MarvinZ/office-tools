...
type AssetStatus = AssetWithEmployee["status"];
type AssetCategory = AssetWithEmployee["category"];
const ALL = "All";
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-zinc-200 p-1 dark:border-zinc-700">
          <button onClick={() => setView("table")} className={`rounded-md p-1.5 transition-colors ${view === "table" ? "bg-zinc-100 text-black dark:bg-zinc-800 dark:text-white" : "text-zinc-400 hover:text-black dark:hover:text-white"}`}>
            <List size={15} />
          </button>
          <button onClick={() => setView("grid")} className={`rounded-md p-1.5 transition-colors ${view === "grid" ? "bg-zinc-100 text-black dark:bg-zinc-800 dark:text-white" : "text-zinc-400 hover:text-black dark:hover:text-white"}`}>
            <LayoutGrid size={15} />
          </button>
        </div>
      </div>
      ...
      {filtered.length !== assets.length && (
        <p className="text-sm text-zinc-400">{t("list.resultsCount", { filtered: filtered.length, total: assets.length })}</p>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                {[t("list.colAsset"), t("list.colCategory"), t("list.colSerial"), t("list.colLocation"), t("list.colStatus"), ""].map((h) => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-zinc-400">{t("list.empty")}</td></tr>
              ) : filtered.map((a) => (
                <tr key={a.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900/50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/assets/${a.id}`} className="text-xs text-zinc-400 hover:text-black dark:hover:text-white">
                      {t("detail.breadcrumb") /* reuse "View →" from detail */}
                    </Link>
                  </td>
                </tr>
              ))}
      ...
      {view === "grid" && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.length === 0 ? (
            <p className="col-span-full text-center text-sm text-zinc-400 py-12">{t("list.empty")}</p>
          ) : filtered.map((a) => (
            <Link key={a.id} href={`/assets/${a.id}`} className="group flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white transition-colors hover:border-zinc-400 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-600">
              <div className="flex h-40 items-center justify-center bg-zinc-100 dark:bg-zinc-800 text-4xl">📦</div>
