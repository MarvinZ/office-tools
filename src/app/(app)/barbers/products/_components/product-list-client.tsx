...
export default function ProductListClient({ products }: { products: ProductRow[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {filtered.length !== products.length && (
        <p className="text-sm text-zinc-400">{t("productsPage.resultsCount", { filtered: filtered.length, total: products.length })}</p>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
              {[t("productsPage.colProduct"), t("productsPage.colPrice"), t("productsPage.colStatus"), ""].map((h, i) => (
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-12 text-center text-sm text-zinc-400">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span>{t("productsPage.empty")}</span>
                  </div>
                </td>
              </tr>
            ) : filtered.map((p) => (
              <tr key={p.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900/50">
                ...
                <td className="whitespace-nowrap px-4 py-3 text-zinc-500">{fmtColones(parseFloat(p.price))}</td>
