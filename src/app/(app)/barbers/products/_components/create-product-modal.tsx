...
export default function CreateProductModal() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      await createProductAction({ name, price: parsedPrice, status: "active" });
      router.refresh();
      resetAndClose();
    });
  }
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 px-4 py-10 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-950">
            <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4 dark:border-zinc-800">
              <h2 className="text-lg font-semibold text-black dark:text-white">{t("productForm.addTitle")}</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-5">
              <div>
                ...
                <input required value={name} onChange={(e) => setName(e.target.value)} placeholder={t("productForm.fieldNamePlaceholder")} className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>{t("productForm.fieldPrice")} *</label>
                <input required type="number" min="0" step="0.01" inputMode="decimal" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.00" className={inputCls} />
                <p className="mt-1 text-xs text-zinc-400">{t("productForm.priceHint")}</p>
              </div>
              <div className="flex justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-800">
                ...
                <button type="submit" disabled={pending} className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-50 dark:bg-white dark:text-black dark:hover:bg-zinc-200">
                  {pending ? tc("saving") : t("productForm.createProduct")}
                </button>
              </div>
            </form>
          </div>
