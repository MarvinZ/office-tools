    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-4 py-10">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          {new Date(data.createdAt).toLocaleDateString("es-CR", {
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total", value: total, color: "text-black dark:text-white" },
      ...
      {data.emails.length === 0 ? (
        <p className="text-sm text-zinc-400">No emails in this batch.</p>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                {["Employee", "Recipient", "Status", "Sent at", "Error"].map((h) => (
                ...
                return (
                  <tr key={email.id} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800">
                    <td className="px-5 py-3 font-medium text-black dark:text-white">
                      {String(payload.empleado ?? "—")}
                    </td>
                    <td className="px-5 py-3 text-zinc-500">{email.recipientEmail}</td>
                    <td className="px-5 py-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${emailStatusStyles[email.status] ?? ""}`}>
                        {email.status}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-zinc-500">
                      {email.sentAt
                        ? new Date(email.sentAt).toLocaleTimeString("es-CR", { hour: "2-digit", minute: "2-digit" })
                        : "—"}
                    </td>
                    <td className="px-5 py-3 text-red-500 text-xs">{email.error ?? "—"}</td>
                  </tr>
                );
              })}
            </tbody>
