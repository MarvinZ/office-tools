...
type Props = {
  ...
  demoPaymentMethods: number;
};

export default function DemoSeedPanel({ tenantId, demoClients, demoProviders, demoEmployees, demoQuotes, demoAssets, demoInvoices, demoBarbers, demoLocations, demoBarberServices, demoPaymentMethods }: Props) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const hasDemo = demoClients > 0 || demoProviders > 0 || demoEmployees > 0 || demoQuotes > 0 || demoAssets > 0 || demoInvoices > 0 || demoBarbers > 0 || demoLocations > 0 || demoBarberServices > 0 || demoPaymentMethods > 0;
